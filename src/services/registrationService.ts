// Cloud Firestore Registration Repository Service for CampusPulse
// Supports Atomic Registration Creation, Unique QR Pass Generation, Realtime Sync, Attendance Marking, and Cancellation
...
async function resolveNumericEventId(idOrUuid: string): Promise<number | null> {
  const parsed = parseInt(idOrUuid, 10);
  if (!isNaN(parsed) && String(parsed) === idOrUuid) {
    return parsed;
  }
  ...
  try {
    ...
    if (res.ok && data.success && data.data?.events) {
      const match = data.data.events.find((e: any) => e.uuid === idOrUuid || String(e.id) === idOrUuid);
      if (match) {
  ...
  if (matchedDigits) {
    return parseInt(matchedDigits[0], 10);
  }
  
  return null;
}

function apiRegistrationToRegistrationDocument(r: any): RegistrationDocument {
  ...
  return {
    ...
    registrationStatus: (r.status === "Confirmed" ? "Approved" : r.status) as any,
    attendanceStatus: r.checked_in ? "Present" : "Pending",
    certificateStatus: r.certificate_token ? "Issued" : "NotIssued",
    qrCode: r.qr_code || "",
    checkedInAt: r.checked_in_time ? new Date(r.checked_in_time).toISOString() : undefined,
...
export const registrationService = {
  ...
  async registerForEvent(params: {
    ...
    department?: string;
    year?: string;
  }): Promise<{ success: boolean; registration?: RegistrationDocument; message?: string }> {
    const { eventId } = params;
    const numericId = await resolveNumericEventId(eventId);

    if (!numericId) {
      toast.error("Registrations are only open for live events.");
      return { success: false, message: "Valid live event ID could not be resolved." };
    }
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        return { success: true, registration: doc };
      }
  ...
  async cancelRegistration(eventId: string, studentId: string): Promise<boolean> {
    ...
    try {
      const res = await apiRequest(`/events/${numericId}/cancel-registration`, {
        method: "DELETE",
      });
  ...
  async updateStatus(registrationId: string, status: RegistrationStatus): Promise<boolean> {
    try {
      ...
      const res = await apiRequest(`/registrations/${registrationId}/${endpoint}`, {
        method: "PATCH",
      });
      if (res.success) {
  ...
  async markAttendance(registrationId: string, attendanceStatus: "Present" | "Absent" | "Pending"): Promise<boolean> {
    try {
      const res = await apiRequest(`/attendance/verify-qr`, {
        method: "POST",
        body: JSON.stringify({
          qr_token: `REG-${registrationId}`,
        }),
      });
      return !!res.success;
    } catch (e) {
  ...
  isRegistered(eventId: string, studentId: string): RegistrationDocument | null {
    return null;
  },

  /** Real-time Registration Subscription via Polling */
  subscribe(options: RegistrationQueryOptions, callback: (registrations: RegistrationDocument[]) => void) {
    let active = true;
    const fetchRegistrations = async () => {
      try {
        ...
        if (options.studentId && !options.studentId.startsWith("std-")) {
          url = `${API_BASE_URL}/students/me/registrations`;
        } else if (options.eventId) {
          const numericId = await resolveNumericEventId(options.eventId);
          if (numericId) {
            url = `${API_BASE_URL}/events/${numericId}/registrations`;
          }
        }
        ...
        if (res.ok && data.success && Array.isArray(data.data) && active) {
          ...
          if (options.status) {
            list = list.filter((r: any) => r.status === options.status);
          }
          callback(list);
        }
      } catch (err) {
